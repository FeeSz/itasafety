  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-ink">Parceiros</h2>
          <p className="text-sm text-ink-muted">Empresas parceiras exibidas na página inicial.</p>
        </div>
        <button
            ...
            <tbody>
              {(data ?? []).map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-ink-muted">{r.tagline || "—"}</td>
...
function PartnerDialog({
  ...
  const save = async () => {
    ...
    try {
      ...
      const { error } = isEdit
        ? await supabase.from("partners").update(payload).eq("id", row!.id)
        ...
        <div className="space-y-4">
          ...
          <FormRow label="URL do Site">
            <input
              ...
              className="input"
              placeholder="ex: https://www.3m.com.br"
            />
          </FormRow>
          <FormRow label="Tagline (Subtítulo)">
            <input
              value={tagline}
              onChange={(e) => setTagline(e.target.value)}
              className="input"
              placeholder="ex: Proteção industrial"
            />
          </FormRow>
